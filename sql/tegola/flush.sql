...
DELETE FROM router WHERE site IN (
	SELECT site.id FROM site
		JOIN network ON site.network = (SELECT id FROM network WHERE name='TEGOLA'));
DELETE FROM radio WHERE antenna IN (
	SELECT antenna.id FROM radio
		...
		JOIN network ON cell.network = (SELECT id FROM network WHERE name='TEGOLA'));
DELETE FROM antenna WHERE cell IN (
	SELECT cell.id FROM cell
		JOIN network ON cell.network = (SELECT id FROM network WHERE name='TEGOLA'));
DELETE FROM site WHERE network = (SELECT id FROM network WHERE name='TEGOLA');
DELETE FROM network WHERE name='TEGOLA';
COMMIT;
